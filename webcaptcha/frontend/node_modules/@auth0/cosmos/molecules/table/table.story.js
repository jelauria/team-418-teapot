"use strict";
var __importStar = (this && this.__importStar) || function (mod) {
    if (mod && mod.__esModule) return mod;
    var result = {};
    if (mod != null) for (var k in mod) if (Object.hasOwnProperty.call(mod, k)) result[k] = mod[k];
    result["default"] = mod;
    return result;
};
Object.defineProperty(exports, "__esModule", { value: true });
var React = __importStar(require("react"));
var react_1 = require("@storybook/react");
var story_helpers_1 = require("../../_helpers/story-helpers");
var __1 = require("../../");
var items = [
    {
        name: 'Isaac Asimov',
        born: 1920,
        died: 1992,
        image: 'data:image/jpeg;base64,'
    },
    {
        name: 'Ray Bradbury',
        born: 1920,
        died: 2012,
        image: 'data:image/jpeg;base64,'
    },
    {
        name: 'Philip K. Dick',
        born: 1928,
        died: 1982,
        image: 'data:image/jpeg;base64,'
    },
    {
        name: 'George Orwell',
        born: 1903,
        died: 1950,
        image: 'data:image/jpeg;base64,'
    }
];
react_1.storiesOf('Table', module).add('default', function () { return (React.createElement(story_helpers_1.Example, { title: "default" },
    React.createElement(__1.Table, { items: items },
        React.createElement(__1.Table.Column, { field: "name", title: "Name" }),
        React.createElement(__1.Table.Column, { field: "born", title: "Born" }),
        React.createElement(__1.Table.Column, { field: "died", title: "Died" })))); });
react_1.storiesOf('Table', module).add('column as variable', function () {
    var columns = (React.createElement(React.Fragment, null,
        React.createElement(__1.Table.Column, { field: "name", title: "Name" }),
        React.createElement(__1.Table.Column, { field: "born", title: "Born" }),
        React.createElement(__1.Table.Column, { field: "died", title: "Died" })));
    return (React.createElement(story_helpers_1.Example, { title: "default" },
        React.createElement(__1.Table, { items: items }, columns)));
});
react_1.storiesOf('Table', module).add('explicit widths', function () { return (React.createElement(story_helpers_1.Example, { title: "default" },
    React.createElement(__1.Table, { items: items },
        React.createElement(__1.Table.Column, { field: "name", title: "Name", width: "70%" }),
        React.createElement(__1.Table.Column, { field: "born", title: "Born" }),
        React.createElement(__1.Table.Column, { field: "died", title: "Died" })))); });
react_1.storiesOf('Table', module).add('cell renderer', function () { return (React.createElement(story_helpers_1.Example, { title: "default" },
    React.createElement(__1.Table, { items: items },
        React.createElement(__1.Table.Column, { field: "image", width: "65px" }, function (item) { return React.createElement(__1.Avatar, { image: item.image, size: "large" }); }),
        React.createElement(__1.Table.Column, { field: "name", title: "Name", width: "65%" }),
        React.createElement(__1.Table.Column, { field: "born", title: "Born" }),
        React.createElement(__1.Table.Column, { field: "died", title: "Died" })))); });
react_1.storiesOf('Table', module).add('sorting', function () { return (React.createElement(story_helpers_1.Example, { title: "default" },
    React.createElement(__1.Table, { items: items },
        React.createElement(__1.Table.Column, { field: "image", width: "65px" }, function (item) { return React.createElement(__1.Avatar, { image: item.image, size: "large" }); }),
        React.createElement(__1.Table.Column, { field: "name", title: "Name", width: "65%", sortable: true }),
        React.createElement(__1.Table.Column, { field: "born", title: "Born", sortable: true }),
        React.createElement(__1.Table.Column, { field: "died", title: "Died", sortable: true })))); });
react_1.storiesOf('Table', module).add('initial sort props', function () { return (React.createElement(story_helpers_1.Example, { title: "default" },
    React.createElement(__1.Table, { items: items, sortOn: "died", sortDirection: "desc" },
        React.createElement(__1.Table.Column, { field: "image", width: "65px" }, function (item) { return React.createElement(__1.Avatar, { image: item.image, size: "large" }); }),
        React.createElement(__1.Table.Column, { field: "name", title: "Name", width: "65%" }),
        React.createElement(__1.Table.Column, { field: "born", title: "Born", sortable: true }),
        React.createElement(__1.Table.Column, { field: "died", title: "Died", sortable: true })))); });
react_1.storiesOf('Table', module).add('stressed', function () { return (React.createElement(story_helpers_1.Example, { title: "stressed - 4 columns with 119 characters per row" },
    React.createElement(__1.Table, { items: [
            {
                data: 'Loremipsumdolorsitametconsecteturadipiscing elit. Cras vehicula massa augue, in consectetur tellus tristique ut.'
            },
            {
                data: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras vehicula massa augue, in consectetur tellus tristique ut.'
            },
            {
                data: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras vehicula massa augue, in consectetur tellus tristique ut.'
            },
            {
                data: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras vehicula massa augue, in consectetur tellus tristique ut.'
            }
        ] },
        React.createElement(__1.Table.Column, { field: "data", title: "Field 1", width: "65px" }),
        React.createElement(__1.Table.Column, { field: "data", title: "Field 2", width: "150px" }),
        React.createElement(__1.Table.Column, { field: "data", title: "Field 3", width: "65px" }),
        React.createElement(__1.Table.Column, { field: "data", title: "Field 4", width: "65px" })))); });
react_1.storiesOf('Table', module).add('stressed with truncating', function () { return (React.createElement(story_helpers_1.Example, { title: "stressed - 7 columns with 119 characters per row" },
    React.createElement(__1.Table, { items: [
            {
                data: 'Loremipsumdolorsitametconsecteturadipiscing Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras vehicula massa augue, in consectetur tellus tristique ut.'
            },
            {
                data: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras vehicula massa augue, in consectetur tellus tristique ut.'
            },
            {
                data: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras vehicula massa augue, in consectetur tellus tristique ut.'
            },
            {
                data: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras vehicula massa augue, in consectetur tellus tristique ut.'
            }
        ] },
        React.createElement(__1.Table.Column, { field: "data", title: "Field 1", width: "150px", truncate: true }),
        React.createElement(__1.Table.Column, { field: "data", title: "Field 2", width: "10%", truncate: true }),
        React.createElement(__1.Table.Column, { field: "data", title: "Field 3", width: "20%", truncate: true }),
        React.createElement(__1.Table.Column, { field: "data", title: "Field 4", width: "auto", truncate: true })))); });
react_1.storiesOf('Table', module).add('with no items', function () { return (React.createElement(story_helpers_1.Example, { title: "no items" },
    React.createElement(__1.Table, { items: [] },
        React.createElement(__1.Table.Column, { field: "image", width: "65px" }, function (item) { return React.createElement(__1.Avatar, { type: "user", image: item.image }); }),
        React.createElement(__1.Table.Column, { field: "name", title: "Name", width: "30%" }),
        React.createElement(__1.Table.Column, { field: "country", title: "Country" }),
        React.createElement(__1.Table.Column, { field: "goals", title: "Goals", sortable: true }),
        React.createElement(__1.Table.Column, { field: "assists", title: "Assists", sortable: true })))); });
react_1.storiesOf('Table', module).add('loading state with data', function () { return (React.createElement(story_helpers_1.Example, { title: "loading" },
    React.createElement(__1.Table, { loading: true, items: [
            {
                data: 'Lorem ipsum dolor sit amet.'
            },
            {
                data: 'Lorem ipsum dolor sit amet.'
            },
            {
                data: 'Lorem ipsum dolor sit amet.'
            },
            {
                data: 'Lorem ipsum dolor sit amet.'
            },
            {
                data: 'Lorem ipsum dolor sit amet.'
            },
            {
                data: 'Lorem ipsum dolor sit amet.'
            },
            {
                data: 'Lorem ipsum dolor sit amet.'
            },
            {
                data: 'Lorem ipsum dolor sit amet.'
            },
            {
                data: 'Lorem ipsum dolor sit amet.'
            },
            {
                data: 'Lorem ipsum dolor sit amet.'
            },
            {
                data: 'Lorem ipsum dolor sit amet.'
            },
            {
                data: 'Lorem ipsum dolor sit amet.'
            }
        ] },
        React.createElement(__1.Table.Column, { field: "data", title: "Field 1" }),
        React.createElement(__1.Table.Column, { field: "data", title: "Field 2" }),
        React.createElement(__1.Table.Column, { field: "data", title: "Field 3" }),
        React.createElement(__1.Table.Column, { field: "data", title: "Field 4" }),
        React.createElement(__1.Table.Column, { field: "data", title: "Field 7" })))); });
react_1.storiesOf('Table', module).add('loading state without data', function () { return (React.createElement(story_helpers_1.Example, { title: "loading" },
    React.createElement(__1.Table, { loading: true, items: [] },
        React.createElement(__1.Table.Column, { field: "data", title: "Field 1" }),
        React.createElement(__1.Table.Column, { field: "data", title: "Field 2" }),
        React.createElement(__1.Table.Column, { field: "data", title: "Field 3" }),
        React.createElement(__1.Table.Column, { field: "data", title: "Field 4" }),
        React.createElement(__1.Table.Column, { field: "data", title: "Field 7" })))); });
react_1.storiesOf('Table', module).add('with buttons', function () { return (React.createElement(story_helpers_1.Example, { title: "with buttons" },
    React.createElement(__1.Table, { items: [
            {
                name: 'Harry Kane',
                goals: 6,
                assists: 0,
                country: '🇬🇧',
                image: 'https://twitter-avatar.now.sh/HKane'
            },
            {
                name: 'Romelu Lukaku',
                goals: 4,
                assists: 1,
                country: '🇧🇪',
                image: 'https://randomuser.me/api/portraits/men/43.jpg'
            },
            {
                name: 'Antoine Griezmann',
                goals: 4,
                assists: 2,
                country: '🇫🇷',
                image: 'https://twitter-avatar.now.sh/AntoGriezmann'
            },
            {
                name: 'Ivan Perišić',
                goals: 3,
                assists: 1,
                country: '🇭🇷',
                image: 'https://twitter-avatar.now.sh/ivanperisic44'
            }
        ], onRowClick: function (evt, item) { return alert(item.name + " was clicked!"); } },
        React.createElement(__1.Table.Column, { field: "name", title: "Name", width: "30%" }, function (item) { return React.createElement(__1.AvatarBlock, { type: "user", image: item.image, title: item.name, size: "compact" }); }),
        React.createElement(__1.Table.Column, { field: "country", title: "Country" }),
        React.createElement(__1.Table.Column, { field: "goals", title: "Goals" }),
        React.createElement(__1.Table.Column, { field: "assists", title: "Assists", width: "5%" }, function (item) { return React.createElement(__1.Button, { appearance: "default", icon: "emails", href: "https://auth0.com" }); })))); });
//# sourceMappingURL=table.story.js.map